...
lineaccesstoken = 't7lnXCKMXQsnr7yvs13gm7Hf/slLWufnln2gvjBTuo5g0Z77BwXANTrahSU/oPZkZtXVzswVzsqrFch46XrxEJVNg8ZslfaF2d6W6xHUWUPU0D/F6MJXlA0P1s0pJd4xElq7CPRkBLwKUybNrMp/fAdB04t89/1O/w1cDnyilFU='
    ...
    if msgType == "text":
        ...
        if(msg == "สวัสดี") :
          ...
          line_bot_api.reply_message(rtoken, replyObj)
            ...
            ''' json_line = request.get_json(force=False,cache=False) json_line = json.dumps(json_line) decoded = json.loads(json_line) ''' crl= pycurl.Curl() crl.setopt( crl.URL, "https://bots.dialogflow.com/line/k--jomf/webhook") crl.setopt( crl.POST, 1) #crl.setopt( crl.BINARYTRANSFER, true) #crl.setopt( crl.POSTFIELDS, json_headers) #crl.setopt( crl.HTTPHEADER, json_headers) #crl.setopt( crl.SSL_VERIFYHOST, 2) #crl.setopt( crl.SSL_VERIFYPEER, 1) #crl.setopt( crl.FOLLOWLOCATION, 1) #crl.setopt( crl.RETURNTRANSFER, 1) crl.perform() crl.close() replyObj = TextSendMessage(text=json_headers) line_bot_api.reply_message(rtoken, replyObj)
        elif msgType == "image":
                try : 
